...
def get_language_unqiue_id(language_ascii_name: str):
    ...
    """
    return int(hashlib.sha1(language_ascii_name.encode("utf-8")).hexdigest(), 16) % 0xFFFF
...
def get_letter_counts(defs: dict, lang: dict, build_version: str) -> Tuple[List[str], Dict[str, int]]:
    text_list = []
    ...
    symbols_by_occurrence = [x[0] for x in sorted(symbol_counts.items(), key=lambda kv: (kv[1], kv[0]), reverse=True)]
...
def get_font_map_per_font(text_list: List[str], fonts: List[str]) -> FontMapsPerFont:
    ...
    if fonts[0] != font_tables.NAME_ASCII_BASIC:
        raise ValueError(f'First item in `fonts` must be "{font_tables.NAME_ASCII_BASIC}"')
    ...
    if total_symbol_count > (0x10 * 0xFF - 15) - 2:  # 4063
        raise ValueError(f"Error, too many used symbols for this version (total {total_symbol_count})")
    ...
    for font in fonts:
        ...
        if len(pending_sym_set) == 0:
            logging.warning(f"Font {font} not used because all symbols already have font bitmaps")
            continue
...
def get_sym_list_and_font_map(text_list: List[str], fonts: List[str]) -> Tuple[List[str], Dict[str, List[str]], FontMap]:
...
def make_font_table_cpp(sym_list: List[str], font_map: FontMap, symbol_map: Dict[str, bytes]) -> str:
    output_table = make_font_table_named_cpp("USER_FONT_12", sym_list, font_map.font12, symbol_map)
...
def make_font_table_06_cpp(sym_list: List[str], font_map: FontMap, symbol_map: Dict[str, bytes]) -> str:
...
def write_bytes_as_c_array(f: TextIO, name: str, data: bytes, indent: int = 2, bytes_per_line: int = 16) -> None:
...
def prepare_language(lang: dict, defs: dict, build_version: str) -> LanguageData:
    ...
    text_list = forced_first_symbols + [x for x in text_list if x not in forced_first_symbols]

    sym_list, sym_lists_by_font, font_map = get_sym_list_and_font_map(text_list, fonts)
    return LanguageData([lang], defs, build_version, sym_list, sym_lists_by_font, font_map)


def prepare_languages(langs: List[dict], defs: dict, build_version: str) -> LanguageData:
    ...
    for lang in langs:
        ...
        for font, font_sym_list in font_maps.sym_lists.items():
            ...
            for sym in font_sym_list:
                font_total_sym_counts[sym] = font_total_sym_counts.get(sym, 0) + sym_counts.get(sym, 0)
    ...
    for font in all_fonts:
        ...
        if font == font_tables.NAME_ASCII_BASIC:
            # We enforce that numbers come first.
            current_sym_list = forced_first_symbols + [x for x in current_sym_list if x not in forced_first_symbols]
...
def write_language(
    ...
    if not compress_font:
        ...
        logging.info(f"Font table 12x16 compressed from {len(font12_uncompressed)} to {len(font12_compressed)} bytes (ratio {len(font12_compressed) / len(font12_uncompressed):.3})")
        write_bytes_as_c_array(f, "font_12x16_brieflz", font12_compressed)
        font_table_text = make_font_table_06_cpp(sym_list, font_map, symbol_conversion_table)
        f.write(font_table_text)
        f.write(
    ...
    translation_common_text = get_translation_common_text(defs, symbol_conversion_table, build_version)
    f.write(translation_common_text)
    f.write(f"const bool HasFahrenheit = {('true' if lang.get('tempUnitFahrenheit', True) else 'false')};\n\n")

    if not strings_bin:
        translation_strings_and_indices_text = get_translation_strings_and_indices_text(lang, defs, symbol_conversion_table)
        ...
        logging.info(f"Strings compressed from {len(strings_bin)} to {len(compressed)} bytes (ratio {len(compressed) / len(strings_bin):.3})")
    ...
    if not strings_bin and not compress_font:
        ...
        f.write("void prepareTranslations() {\n")
        if compress_font:
            f.write("  blz_depack_srcsize(font_12x16_brieflz, font_out_buffer, sizeof(font_12x16_brieflz));\n")
        if strings_bin:
            f.write("  blz_depack_srcsize(translation_data_brieflz, translation_data_out_buffer, sizeof(translation_data_brieflz));\n")
        f.write("}\n\n")
...
def write_languages(
    ...
    lang_names = [lang.get("languageLocalName", lang["languageCode"]) for lang in data.langs]
    ...
    if not compress_font:
        font_table_text = ""
        font_section_info_text = "const FontSectionDataInfo FontSectionDataInfos[] = {\n"
        ...
        for font, current_sym_list in sym_lists_by_font.items():
            ...
            logging.info(f"Font table for {font} compressed from {len(font_uncompressed)} to {len(font_compressed)} bytes (ratio {len(font_compressed) / len(font_uncompressed):.3})")
    ...
    translation_common_text = get_translation_common_text(defs, symbol_conversion_table, build_version)
    f.write(translation_common_text)
    f.write(f"const bool HasFahrenheit = {('true' if any([lang.get('tempUnitFahrenheit', True) for lang in data.langs]) else 'false')};\n\n")

    max_decompressed_translation_size = 0
    if not strings_obj_path:
        for lang in data.langs:
            lang_code = lang["languageCode"]
            translation_strings_and_indices_text = get_translation_strings_and_indices_text(lang, defs, symbol_conversion_table, suffix=f"_{lang_code}")
            f.write(translation_strings_and_indices_text)
        ...
        for lang in data.langs:
            ...
            max_decompressed_translation_size = max(max_decompressed_translation_size, len(strings_bin))
            compressed = brieflz.compress(strings_bin)
            logging.info(f"Strings for {lang_code} compressed from {len(strings_bin)} to {len(compressed)} bytes (ratio {len(compressed) / len(strings_bin):.3})")
            write_bytes_as_c_array(f, f"translation_data_brieflz_{lang_code}", compressed)
        f.write("const LanguageMeta LanguageMetas[] = {\n")
        for lang in data.langs:
            lang_code = lang["languageCode"]
            lang_id = get_language_unqiue_id(lang_code)
...
def get_translation_common_text(defs: dict, symbol_conversion_table: Dict[str, bytes], build_version) -> str:
    ...
    for c in get_debug_menu():
        translation_common_text += f'\t "{convert_string(symbol_conversion_table, c)}",//{c} \n'
    translation_common_text += "};\n\n"
...
def get_translation_strings_and_indices_text(lang: dict, defs: dict, symbol_conversion_table: Dict[str, bytes], suffix: str = "") -> str:
    str_table: List[str] = []
    ...
    for index, mod in enumerate(defs["menuOptions"]):
        eid = mod["id"]
        str_group_settingdesc.append(TranslationItem(f"[{index:02d}] {eid}", len(str_table)))
    ...
    for index, mod in enumerate(defs["menuOptions"]):
        ...
        if isinstance(obj[eid]["text2"], list):
            ...
            source_text = "\n" + obj[eid]["text2"]
        str_group_settingshortnames.append(TranslationItem(f"[{index:02d}] {eid}", len(str_table)))
        str_table.append(source_text)
    ...
    for index, mod in enumerate(defs["menuGroups"]):
        ...
        if isinstance(obj[eid]["text2"], list):
            ...
            source_text = "\n" + obj[eid]["text2"]
        str_group_settingmenuentries.append(TranslationItem(f"[{index:02d}] {eid}", len(str_table)))
        str_table.append(source_text)
    ...
    for index, mod in enumerate(defs["menuGroups"]):
        eid = mod["id"]
        str_group_settingmenuentriesdesc.append(TranslationItem(f"[{index:02d}] {eid}", len(str_table)))
        str_table.append(obj[eid]["desc"])
    ...
    backward_sorted_table: List[Tuple[int, str, bytes]] = sorted(
        ((i, s, bytes(reversed(convert_string_bytes(symbol_conversion_table, s)))) for i, s in enumerate(str_table)),
        key=lambda x: x[2],
    )
    ...
    for i, source_str in enumerate(str_table):
        ...
        str_used_by = [i] + [j for j, r in enumerate(str_remapping) if r and r.str_index == i]
...
def get_translation_sanity_checks_text(defs: dict) -> str:
    ...
    for i, mod in enumerate(defs["menuOptions"]):
        eid = mod["id"]
        sanity_checks_text += f"static_assert(static_cast<uint8_t>(SettingsItemIndex::{eid}) == {i});\n"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", "-o", help="Target file", type=argparse.FileType("w"), required=True)
    parser.add_argument(
        "languageCodes",
...
def main() -> None:
    ...
    if args.input_pickled:
        ...
        if language_codes != args.languageCodes:
            logging.error(f"error: languageCode {args.languageCode} does not match language data {language_codes}")
            sys.exit(1)
        ...
        if len(args.languageCodes) == 1:
            ...
            langs_ = [read_translation(json_dir, lang_code) for lang_code in args.languageCodes]
