...
void setCalibrationOffset(int16_t offSet) { CalibrationTempOffset = offSet; }
...
uint16_t tipMeasurementToC(uint16_t raw) {
  ...
  uint32_t tipDelta = ((raw - CalibrationTempOffset) * tipGainCalValue) / 1000;
...
uint16_t ctoTipMeasurement(uint16_t temp) {
  ...
  int32_t TipRaw =
      ((temp - (getHandleTemperature() / 10)) * 10000) / tipGainCalValue;
...
uint16_t __attribute__((long_call, section(".data.ramfuncs")))
...
uint16_t getTipRawTemp(uint8_t instant) {
  ...
  const uint8_t filterBeta =
      7;  // higher values smooth out more, but reduce responsiveness
...
#define BATTFILTERDEPTH 64
  ...
  if (preFillneeded) {
    for (uint8_t i = 0; i < BATTFILTERDEPTH; i++) samples[i] = getADC(1);
    preFillneeded = 0;
  }
  ...
  for (uint8_t i = 0; i < BATTFILTERDEPTH; i++) sum += samples[i];

  sum /= BATTFILTERDEPTH;
  if (sum < 50) preFillneeded = 1;
  return sum * 4 / divisor;
}
#ifdef MODEL_TS80
uint8_t QCMode = 0;
void seekQC(int16_t Vx10) {
  if (QCMode <= 1) return;  // NOT connected to a QC Charger
  if (Vx10 < 50) return;
...
void startQC() {
  ...
  if (vin > 150) return;  // Over voltage
  if (vin > 100) {
  ...
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_3,
                    GPIO_PIN_RESET);  // pull low to put 0.6V on D+
  ...
  if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_11) == GPIO_PIN_RESET) enteredQC = 1;
  if (enteredQC) {
...
uint8_t getTipPWM() { return pendingPWM; }
void __attribute__((long_call, section(".data.ramfuncs")))
setTipPWM(uint8_t pulse) {
  PWMSafetyTimer =
      2;  // This is decremented in the handler for PWM so that the tip pwm is
