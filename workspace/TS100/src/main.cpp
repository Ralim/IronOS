...
ButtonState getButtonState() {
  ...
  if (currentState) lastButtonTime = xTaskGetTickCount();
  if (currentState == previousState) {
    ...
    if (currentState) {
      ...
      if (currentState != previousState) {
        ...
        // both at once
        if (previousState == 0x03) currentState = 0x03;
      }
    } else {
...
void waitForButtonPressOrTimeout(uint32_t timeout) {
  ...
  while (buttons) {
    buttons = getButtonState();
    GUIDelay();
    if (xTaskGetTickCount() > timeout) return;
  }
  while (!buttons) {
    buttons = getButtonState();
    GUIDelay();
...
static void display_countdown(int sleepThres) {
   ...
   */
  int lastEventTime =
      lastButtonTime < lastMovementTime ? lastMovementTime : lastButtonTime;
...
static const char *HEADERS[] = {
    __DATE__, "Heap: ", "HWMG: ", "HWMP: ", "HWMM: ", "Time: ",
    "Move: ", "RTip: ", "CTip: ", "Vin :",  "THan: ", "Model: "};
...
void startGUITask(void const *argument __unused) {
  ...
  if (systemSettings.autoStartMode) {
    ...
    if (systemSettings.autoStartMode == 1) gui_solderingMode(0);
    if (systemSettings.autoStartMode == 2) gui_solderingMode(1);
  }
...
//^ Kept here for a way to block this thread
  ...
  for (;;) {
      ...
      case BUTTON_F_SHORT:
        ...
        if (idealQCVoltage < 90)
          idealQCVoltage = calculateMaxVoltage(
              1);  // 1 means use filtered values rather than do its own
    ...
    uint16_t tipTemp = tipMeasurementToC(
        getTipRawTemp(1));  // This forces a faster update rate on the filtering
    ...
    if (systemSettings.detailedIDLE) {
      ...
      if (OLED::getRotation()) {
        ...
        OLED::drawArea(0, 0, 84, 16,
                       idleScreenBGF);  // Needs to be flipped so button ends up
...
#endif
  currentlyActiveTemperatureTarget =
      0;  // Force start with no output (off). If in sleep / soldering this will
  ...
  for (;;) {
    if (ulTaskNotifyTake(pdTRUE, 50)) {
      ...
      if (currentlyActiveTemperatureTarget) {
        ...
        if (((int32_t)systemSettings.PID_I)) output += integralCount;
        ...
        setTipPWM(
            0);  // disable the output driver if the output is set to be off
        integralCount = 0;
        derivativeLastValue = 0;
      }
...
void startMOVTask(void const *argument __unused) {
  ...
  seekQC(
      idealQCVoltage);  // this will move the QC output to the preferred voltage
